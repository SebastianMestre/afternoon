...
// add random springs to make it less like a crystal structure. Should dampen vibrations.
for (let i = 0; i < 50; ++i) {
	let a = Math.floor(Math.random() * 11 * 11);
	let b = a;
	while (a == b) {
		b = Math.floor(Math.random() * 11 * 11);
	}
	const p1 = particles[a];
	const p2 = particles[b];
	springs.push(spring(p1, p2, distance(p1, p2), 100.0));
}
